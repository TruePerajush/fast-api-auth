...
async def get_db_engine() -> AsyncGenerator[AsyncEngine]:
    if _engine is None:
        raise RuntimeError(
            "Database engine not initialized. Call init_db_engine() at startup."
        )
    yield _engine
...
async def get_db_session() -> AsyncGenerator[AsyncSession]:
    if _session_maker is None:
        raise RuntimeError(
            "Database session maker not initialized. Call init_db_engine() at startup."
        )
...
async def create_tables():
    if _engine is None:
        raise RuntimeError(
            "Database engine not initialized. Call init_db_engine() first."
        )
